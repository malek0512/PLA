...
import model.parser.*;
import controller.TableTransitionSortie.Triplet;
	...
	public Automate.Entree getEntree() throws Exception{
		...
		for (Iterator<Automate.Entree> key = entries.keySet().iterator(); key.hasNext(); ){
			...
			//if ( entries.get(Entree).ok ){
				...
				case CASE_LIBRE: if (primitivesTest.configCaseDevant()==Entree.CASE_LIBRE) return controller.Automate.Entree.CASE_LIBRE; break;
				...
				case SORTIE_TERRAIN: if (primitivesTest.configCaseDevant()==controller.Automate.Entree.SORTIE_TERRAIN) return Entree.SORTIE_TERRAIN;break;
